...
contract PaymentSystem {
    ...
    enum PaymentType { PRODUCT, DONATION, SUBSCRIPTION }
    ...
    function payForProduct(
        ...
        if(tokenAddress == address(0)) {
            ...
            uint256 finalAmount = msg.value - commission;
            ...
            uint256 commission = calculateCommission(amount);
            uint256 finalAmount = amount - commission;
            
            // Переводим токены
            require(token.transferFrom(msg.sender, commissionWallet, commission), "Commission transfer failed");
            require(token.transferFrom(msg.sender, recipient, finalAmount), "Payment transfer failed");
    ...
    function makeDonation(
        ...
        if(tokenAddress == address(0)) {
            ...
            require(token.transferFrom(msg.sender, commissionWallet, commission), "Commission transfer failed");
            require(token.transferFrom(msg.sender, recipient, finalAmount), "Donation transfer failed");
